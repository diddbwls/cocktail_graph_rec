...
from config import get_config, get_c4_config
...
class C4Retrieval(BaseRetrieval):
    ...
    def retrieve(self, user_question: str) -> List[Dict[str, Any]]:
        ...
        # 4단계: 상위 결과 선택
        final_cocktail_names = [item['name'] for item in similar_cocktails[:self.c4_config['final_top_k']]]
        print(f"4단계 - 최종 선정: {len(final_cocktail_names)}개")
        
        for i, item in enumerate(similar_cocktails[:self.c4_config['final_top_k']], 1):
